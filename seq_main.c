...
int main(int argc, char **argv) {
	...
	double  timing, io_timing, io_timing_read, io_timing_write, clustering_timing;
	...
	while ( (opt=getopt(argc,argv,"i:c:n:t:s:doqh"))!= EOF) {
		switch (opt) {
	...
	if (is_output_timing) {
		...
		printf("I/O time (read phase): %.4f s\n", io_timing_read);
	...
	seq_kmeans(objects, numCoords, numObjs, numClusters, threshold, membership, clusters, is_output_timing);
	...
	if (is_output_timing) {
		io_timing_write = wtime() - io_timing_write;
